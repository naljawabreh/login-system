...
export class AuthService {
  ...
  async register(
    ...
    this.logger.log(
      `AuthService: register user with identifier: ${registerDto.email}`,
    );
    const { email, password, firstName, lastName, phoneNumber, isResident } =
      registerDto;
  ...
  async regenerateOtp(userMail: string): Promise<registrationResponseDto> {
    this.logger.log(
      `AuthService: regenerateOtp user with identifier: ${userMail}`,
    );
  ...
  async validateUser(loginDto: LoginDto): Promise<UserDocument | null> {
    this.logger.log(
      `AuthService: Validating user with identifier: ${loginDto.identifier}`,
    );
    const user = await this.usersService.findOneByEmailOrPhoneNumber(
      loginDto.identifier,
    );
    if (user) {
      ...
      if (isMatch) {
        this.logger.log(
          `AuthService: Password matched for user: ${user.email}`,
        );
        return user;
      } else {
      ...
      this.logger.warn(
        `User not found with identifier: ${loginDto.identifier}`,
      );
    }
    return null;
  }
  ...
  async login(loginDto: LoginDto): Promise<LoginResponseDto> {
    this.logger.log(
      `AuthService: Attempting login for identifier: ${loginDto.identifier}`,
    );
    const user = await this.validateUser(loginDto);
    if (!user) {
      this.logger.warn(
        `Invalid credentials for identifier: ${loginDto.identifier}`,
      );
  ...
  async verifyOtp(userMail: string, otp: string): Promise<LoginResponseDto> {
    this.logger.log(
      `AuthService: verify OTP for a user with mail: ${userMail}`,
    );
  ...
  async findByEmailSendOTP(userMail: string): Promise<void> {
    this.logger.log(
      `AuthService: findByEmailSendOTP for a user with mail: ${userMail}`,
    );
    const user = await this.usersService.findOneByEmail(userMail);
  ...
  async findEmailValidateOTP(
    userMail: string,
    otp: string,
  ): Promise<LoginResponseDto> {
    this.logger.log(
      `AuthService: findEmailValidateOTP for a user with mail: ${userMail}`,
    );
    const user = await this.usersService.findOneByEmail(userMail);
  ...
  async findUserValidateOTP(
    userMail: string,
    otp: string,
  ): Promise<UserDocument> {
    this.logger.log(
      `AuthService: findUserValidateOTP for a user with mail: ${userMail}`,
    );
    const user = await this.usersService.findOneByEmail(userMail);
  ...
  async updateUser(
    ...
    this.logger.log(
      `AuthService: updateUser for a user with mail: ${userMail}`,
    );
    const user = await this.usersService.findOneByEmail(userMail);
    ...
    if (updateUserDto.phoneNumber) {
      const existingUserWithPhoneNumber =
        await this.usersService.findOneByPhoneNumber(updateUserDto.phoneNumber);

      // Ensure the found user is not the same as the current user
      if (
        existingUserWithPhoneNumber &&
        existingUserWithPhoneNumber.email !== userMail
      ) {
